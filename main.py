...
class Game:
    ...
    def finish_turn(self):
        """
        change la variable `current_player`
        ajoute 1 a mana_max pour le prochain joueur
...
class Player:
    ...
    def load_deck(self):
        """
        charge le deck depuis un fichier json
        """
        f = open(DECK_FILE, "r")
    ...
    def has_taunt(self):
        """
        retourn un bool; true si une des carte dans le battlefield a taunt, false sinon
        """
    ...
    def hide_hand(self):
        """
            cache chaque carte de la main du joueur
        """
    ...
    def draw_hand(self):
        ...
        les coordonée sont predefinie en fonction de la longeur de la longeur du joueur
        """
        if len(self.hand) == 1:
    ...
    def draw_ally_bf(self):
        """
        dessine le battlefield au dessu de la main
        """
    ...
    def draw_ally_info(self):
        """
        dessine la vie et la mana au niveau de la main
        """
    ...
    def draw_en_bf(self):
        """
        dessine le battlefield en haut de l'ecran
        """
        offset = 52 if len(self.battlefield) % 2 == 0 else 106
        for i in range(len(self.battlefield)):
            c_offset = 50 * ((8 if len(self.battlefield) % 2 == 0 else 7)
    ...
    def draw_en_info(self):
        """
        dessine la vie et la mana au en haut de l'écran
        """
        self.canv.itemconfig(self.h_id, text=self.health, anchor=NW)
        self.canv.itemconfig(self.m_id, text="{}/{}".format(self.mana, self.mana_max),
                             anchor=NW)
    ...
    def clear_dead(self):
        """
        retire toute les carte qui ont moins de 1 vie
        [UNUSED]
        """
...
class Card:
    ...
    def remove_attack(self):
        """
        La carte ne peut plus attaquer
        """
        self.can_attack = False
        return self
    ...
    def draw(self, x, y):
        """
            dessine la carte sur le canvas au coordonée (x,y) avec les texts
        """
        self.coords = (x, y)
    ...
    def is_clicked(self, coords):
        """
        retourne un bool; indique si les coordonée donnée sont sur la carte
    ...
    def battle(self, card):
        """
        fait une battaille entre deux carte
        """
